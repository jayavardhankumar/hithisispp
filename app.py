...
def formDirectImg(filename,dirname):
    ...
    cv2.imwrite(f"D:\PROJECT_AND_CODES\KYC_VERIFICATION\\imgdatabase{dirname}\\Dataset\\img0"+".png",image)
...
def compare(dirname):
    ...
    global count1
    print('Count1 : ',count1)
    p=open('D:\PROJECT_AND_CODES\KYC_VERIFICATION\\dirname.txt','w+')
    p.write(dirname)
    for j in range(2):
        print('Path1 '+str(j))
...
def scan_qr():
    ...
    img=cv2.imread(f"D:\PROJECT_AND_CODES\KYC_VERIFICATION\\imgdatabase{dirname}\\Dataset\\img0.png")
    code=decode(img)
    dat=''
    print("\n1\n")
    for i in code:
